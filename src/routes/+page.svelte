<script lang="ts">
	...
	// Streaming: Fetch breaking news and prepend to politics
	async function loadBreakingNews() {
		try {
			const breakingItems = await fetchBreakingNews();
			if (breakingItems.length > 0) {
				// Prepend to politics category
				news.appendItems('politics', breakingItems);
			}
		} catch (error) {
	...
	onMount(() => {
		...
		refresh.setupStreaming(loadBreakingNews);
...
<style>
	...
	.right-map :global(.panel-content) {
		flex: 1;
		overflow: hidden;
		min-height: 0;
		position: relative;
